...
public class SwaggerConfig {
	...
	public Docket customImplementation() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).securitySchemes(newArrayList(apiKey()))
				...
				// .apis(RequestHandlerSelectors.any()) // If you want to list
				// all the apis including springboots own
				.apis(RequestHandlerSelectors.basePackage("com.app.api")).build().pathMapping("/")
	...
	private ApiKey apiKey() {
		...
		return new ApiKey("Authorization", "", "header"); // <<< === Create a
															// Heaader (We are
															// createing header
															// named
															// "Authorization"
	...
	SecurityConfiguration security() {
		// return new SecurityConfiguration("emailSecurity_client", "secret",
		// "Spring", "emailSecurity", "apiKey", ApiKeyVehicle.HEADER, "api_key",
		// ",");
